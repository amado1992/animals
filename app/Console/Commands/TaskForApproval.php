...
class TaskForApproval extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:task-for-approval';
     ...
     * @var string
     */
    protected $description = 'Send list of new tasks for approval';
    ...
    public function handle()
    {
        ...
        foreach ($users as $row) {
            $tasks = Task::where('send_forapproval', 1)
    ...
    public function sendTaskEmail($tasks, $user = 'johnrens@zoo-services.com', $create = false)
    {
        if (count($tasks) > 0) {
            ...
            $email_subject = 'Tasks for approval for checking';
            $email_title   = "Tasks for approval for checking";
            $complete      = false;
