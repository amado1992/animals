...
class ListNewWantedCreated extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:list-wanted-created';
     ...
     * @var string
     */
    protected $description = 'Send list of new wanted created';
    ...
    {
        $wanteds = Wanted::where('new_wanted', 1)
            ->orderBy("created_at", "DESC")
            ->get();

        if (count($wanteds) > 0) {
            $email_from    = 'info@zoo-services.com';
            $email_subject = 'New Wanted';
            $email_title   = "New Wanted";

            $email_body = view('emails.list-wanted-new', compact('wanteds', 'email_title'))->render();
            ...
            $this->info('Successfully sent wanted email to members.');
        } else {
            $this->info('There are not wanted to send.');
        }
    }
    ...
    public function createSentEmail($subject, $from, $email, $body)
    {
        ...
        $dashboard = Dashboard::where("filter_data", "new_inserted_wanted")->first();
        if (!empty($dashboard)) {
