...
class ListNewOfferSendOut extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:list-offers-send-out';
     ...
     * @var string
     */
    protected $description = 'Send list of new offers send out';
    ...
    {
        $offers_filter = Offer::where('new_offer_send', 1)
        ...
        if (count($offers) > 0) {
            ...
            $email_subject = 'Offers sent out';
            $email_title   = "Offers sent out";
            $date_send     = false;
            $offer_send    = true;

            $email_body = view('emails.list-offer-send-out', compact('offers', 'email_title', 'date_send', 'offer_send'))->render();
    ...
    public function createSentEmail($subject, $from, $email, $body)
    {
        ...
        $dashboard = Dashboard::where("filter_data", "offers_sent")->first();
        if (!empty($dashboard)) {
