...
class ListNewOfferForApproval extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:list-offers-for-approval';
     ...
     * @var string
     */
    protected $description = 'Send list of new offers for approval';
    ...
    {
        $offers_filter = Offer::where('new_offer_forapproval', 1)
        ...
        if (count($offers) > 0) {
            ...
            $email_subject = 'Offers for approval';
            $email_title   = "Offers for approval";
            $date_send     = false;
    ...
    public function createSentEmail($subject, $from, $email, $body)
    {
        ...
        $dashboard = Dashboard::where("filter_data", "offers_approve")->first();
        if (!empty($dashboard)) {
