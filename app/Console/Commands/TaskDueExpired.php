...
class TaskDueExpired extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:task-due-expired';
    ...
    public function sendTaskEmail($tasks, $user = 'johnrens@zoo-services.com', $create = false)
    {
        if (count($tasks) > 0) {
            ...
            $email_subject = 'Tasks expired for checking';
            $email_title   = "Tasks expired for checking";
            $complete      = false;
