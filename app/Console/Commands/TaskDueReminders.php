...
class TaskDueReminders extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:task-due-reminders';
     ...
     * @var string
     */
    protected $description = 'Send list of tasks due reminders';
    ...
    public function handle()
    {
        ...
        foreach ($users as $row) {
            $tasks = Task::whereDate('due_date', Carbon::now()->format('Y-m-d'))
    ...
    public function sendTaskEmail($tasks, $user = 'johnrens@zoo-services.com', $create = false)
    {
        if (count($tasks) > 0) {
            ...
            $email_subject = 'Tasks Due Reminders for checking';
            $email_title   = "Tasks Due Reminders for checking";
            $complete      = false;
    ...
    public function createSentEmail($subject, $from, $email, $body)
    {
        ...
        $dashboard = Dashboard::where("filter_data", "tasks_over_time_remind")->first();
        if (!empty($dashboard)) {
