            ...
            <div class="card-body">
                ...
                <div class="inbox-leftbar">
                    ...
                    <div class="mail-list mt-4">
                        <a href="{{ route('inbox.index') }}" class="dropdown-item {{ $type_page == "inbox" && $label == "false" ? "active" : "" }}"><i class="ri-inbox-fill align-bottom me-2"></i>Inbox @if($total_unread)<span class="badge badge-soft-danger float-end ms-2 total-inbox" style="color: #fff;background: #f1556c;">{{ $total_unread }}</span>@endif</a>
                        <a href="{{ route('inbox.index') }}?is_send=1" class="dropdown-item {{ $type_page == "sentitems" ? "active" : "" }}"><i class="ri-send-plane-2-line align-bottom me-2"></i>Sent Mail</a>
                        <a href="{{ route('inbox.index') }}?archive=1" class="dropdown-item {{ $type_page == "archive" ? "active" : "" }}"><i class="mdi mdi-archive align-bottom me-2"></i>Archive</a>
                        <a href="{{ route('inbox.index') }}?is_spam=1" class="dropdown-item {{ $type_page == "junkemail" ? "active" : "" }}"><i class="ri-error-warning-line align-bottom me-2"></i>Spam</a>
                        <a href="{{ route('inbox.index') }}?is_draft=1" class="dropdown-item {{ $type_page == "drafts" ? "active" : "" }}"><i class="mdi mdi-file align-bottom me-2"></i>Draft</a>
                        <a href="{{ route('inbox.index') }}?is_delete=1" class="dropdown-item {{ $type_page == "deleteditems" ? "active" : "" }}"><i class="ri-delete-bin-line align-bottom me-2"></i>Trash</a>
                    </div>
                    @if (!empty($labels))
                ...
                <div class="inbox-rightbar">
                    <div class="row scroll-option-bar">
                        <div class="col-md-12">
                                    ...
                                    <div class="scrroll_style">
                                        @foreach ($colors as $row)
                                            <a class="dropdown-item addEmailColor" data-id="{{ $row->id }}" href="#">
                                                    ...
                                                    <span class="mdi mdi-circle me-2" style="color: {{ $row->color }}"></span>{{ $row->title }}
                            ...
                            <div class="btn-group me-1">
                                <button type="button" id="deleteLabelSelectedEmail" class="btn btn-sm btn-light dropdown-toggle waves-effect" data-toggle="dropdown" aria-expanded="false" title="Delete Label">
                                    <i class="mdi mdi-delete-variant font-18"></i>
                                    <i class="mdi mdi-label font-18"></i>
                                    <i class="mdi mdi-chevron-down"></i>
                                </button>
                                <div class="dropdown-menu">
                                    <span class="dropdown-header">Delete Label as:</span>
                                    @foreach ($labels as $row )
                                        <a class="dropdown-item deleteLabelSelectedEmail" href="#" data-id="{{ $row["id"] }}"><span class="mdi mdi-circle me-2" style="color: {{ $row["color"] }}"></span>{{ $row["title"] }}</a>
                                    @endforeach
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    function assingOfferEmail(id) {
        ...
        if(ids.length == 0) {
            ...
            // Make sure cache of select client is empty
            $('.contact-select2-inbox.contact-select2-client-offer').val(null).trigger('change');
            ...
            $.ajax({
                ...
                success:function(data) {
                    if(data.error){
                        ...
                        if(typeof data.items[0] !== 'undefined' && data.items[0].id != '') {
                           // Create a DOM Option and pre-select by default
                           var newOption = new Option(data.items[0].email, data.items[0].id, false, false);
                           $('.contact-select2-inbox.contact-select2-client-offer').append(newOption).trigger('change');
                        }
                        ...
                        $.ajax({
                            ...
                            success: function (r) {
                                console.log(r);
                                if(r.error){
                                    $('#assing-offer-data').html("<p class='norecselected'>No offer to show. Please refine your search via filters above.</p>");
                                }else{
                                    $('#assing-offer-data').html(r.content);
                                }
                                if(typeof data.items[0] !== 'undefined' && data.items[0].id != ''){
                                    $('span[id^=select2-filter_client_id-]').text(data.items[0].email);
                                 }
                            }
                        });
                    }
    ...
    function assingOrderEmail(id) {
        ...
        var ids = [];
        var email = $(".assing_order_email_" + id).attr("data-email");
        if(typeof dataId != 'undefined'){
            ids.push(dataId);
        }else{
            $(":checked.selector").each(function(){
        ...
        if(ids.length == 0) {
            ...
            // Make sure cache of select client is empty
            $('.contact-select2-inbox.contact-select2-client-order').val(null).trigger('change');
                ...
                success:function(data) {
                    if(data.error){
                        ...
                        if(typeof data.items[0] !== 'undefined' && data.items[0].id != '') {
                           // Create a DOM Option and pre-select by default
                           var newOption = new Option(data.items[0].email, data.items[0].id, false, false);
                           $('.contact-select2-inbox.contact-select2-client-order').append(newOption).trigger('change');
                        }
                        ...
                        $.ajax({
                            ...
                            success: function (r) {
                                console.log(r);
                                if(r.error){
                                    $('#assing-order-data').html("<p class='norecselected'>No order to show. Please refine your search via filters above.</p>");
                                }else{
    ...
    function archiveEmailsProject(id) {
        var dataId = $(".archive_emails_" + id).attr('data-id');
        var ids = [];
        var account = $("#acount_show").val();
        var scroll_position = $(document).scrollTop();
        ...
        if(ids.length == 0)
            ...
            $(".archive_emails_" + id).html('<span class="spinner-border spinner-border-sm" style="margin: 0px !important;" role="status"></span>');
            $.ajax({
                type:'POST',
                url:"{{ route('inbox.archiveItems') }}",
                data:{items: ids, account: account},
    ...
    $('.deleteLabelSelectedEmail').on('click', function () {
        var ids = [];
        var _dt = $(this);
        $(":checked.selector").each(function(){
            ids.push($(this).val());
        ...
        var label = _dt.attr("data-id");
        var label_text = _dt.html();

        if(ids.length == 0){
            alert("You must select items to delete labels.");
        } else {
            Swal.fire({
                title: "Are you sure?",
                text: 'The label will be removed from the selected emails!',
                icon: "warning",
                buttons: true,
                dangerMode: true,
                showCancelButton: true,
            ...
            .then((result) => {
                if(typeof result != "undefined" && result.isConfirmed){
                    $('#deleteLabelSelectedEmail').html('<span class="spinner-border spinner-border-sm" role="status"></span>');

                    $.ajax({
                        type:'POST',
                        url:"{{ route('inbox.deleteLabels') }}",
                        data:{items: ids, label: label},
                        dataType: "JSON",
                        success: function(data){
                            if(data.error){
...
//
$("input[name='filter_request_number'], input[name='filter_order_number']").on('change', function (e) {
   $('#assing-offer-form, #assing-order-form').on('submit', function(event) {
      // prevent the default behavior of the form submission, only for this field
        ...
        if(!url.includes("acount_show")){
            ...
            if(type_page !== "inbox"){
                if(type_page === "deleteditems"){
                    url = "{{route('inbox.index')}}?is_delete=1&acount_show=" + acount_show;
                }
                else if(type_page === "drafts"){
                    url = "{{route('inbox.index')}}?is_draft=1&acount_show=" + acount_show;
                }else if(type_page === "sentitems"){
                    url = "{{route('inbox.index')}}?is_send=1&acount_show=" + acount_show;
                }else{
                    url = "{{route('inbox.index')}}?" + type_page +"=1&acount_show=" + acount_show;
                }
            }else{
                url = "{{route('inbox.index')}}?acount_show=" + acount_show;
            }
    ...
    $(document).on("scroll", function(){
        ...
        if(scroll_position >= 100){
            ...
            $("#list-content-message").addClass("scroll-option-content-space");
        }else{
            $(".scroll-option-bar").removeClass("scroll-option-bar-fixed");
            $("#list-content-message").removeClass("scroll-option-content-space");
        }
    });
